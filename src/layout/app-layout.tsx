...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['account-owner', 'family-member', 'admin'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<SubscriberInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/subscribers` : null),
    () => getSubscribers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Subscriber',
      path: isTenantUser ? `/subscribers/view/${data?.[0]?.id}` : '/subscribers',
      entity: 'subscriber',
